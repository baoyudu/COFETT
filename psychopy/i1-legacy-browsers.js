...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'split_data_1.xlsx', 'path': 'split_data_1.xlsx'},
  ]
});
...
function instructionRoutineEachFrame() {
  return async function () {
    ...
    if (key_resp_2.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_2.getKeys({keyList: ['return'], waitRelease: true});
...
function trials_2LoopBegin(trials_2LoopScheduler, snapshot) {
  return async function() {
    ...
    trials_2.forEach(function() {
      ...
      trials_2LoopScheduler.add(recallRoutineBegin(snapshot));
      trials_2LoopScheduler.add(recallRoutineEachFrame());
      trials_2LoopScheduler.add(recallRoutineEnd(snapshot));
...
function trialsRoutineEachFrame() {
  return async function () {
    ...
    if (t > trialsMaxDuration) {
        trialsMaxDurationReached = true
        continueRoutine = false
    }
...
function recallRoutineEachFrame() {
  return async function () {
    ...
    // update/draw components on each frame
    // is it time to end the Routine? (based on local clock)
    if (t > recallMaxDuration) {
        recallMaxDurationReached = true
        continueRoutine = false
    }
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
...
function recallRoutineEnd(snapshot) {
  return async function () {
    ...
    if (recallMaxDurationReached) {
        recallClock.add(recallMaxDuration);
    } else {
        recallClock.add(3.300000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
...
function blankRoutineBegin(snapshot) {
  return async function () {
    ...
    blankMaxDuration = 1.8
    // keep track of which components have finished
    blankComponents = [];
    blankComponents.push(text_5);
...
function blankRoutineEachFrame() {
  return async function () {
    ...
    // update/draw components on each frame
    // is it time to end the Routine? (based on local clock)
    if (t > blankMaxDuration) {
        blankMaxDurationReached = true
        continueRoutine = false
    }
...
function endRoutineEachFrame() {
  return async function () {
    ...
    if (key_resp_3.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_3.getKeys({keyList: ['return'], waitRelease: true});
